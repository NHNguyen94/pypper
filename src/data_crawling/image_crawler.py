...
class ImageCrawler(BaseCrawler):
    ...
    def fetch_one(self,
        ...
        :param substring_condition: The condition that the image url should contain.
        :param driver: The webdriver object, you might need to download the driver at https://chromedriver.chromium.org/downloads
        :param endswith_condition: The condition that the image url should end with.
        :param startswith_condition: The condition that the image url should start with.
        :param driver_wait_time: The time to wait for the driver to load the page.
        ...
        try:
            ...
            for img in soup.findAll(self.configs.IMG_TAG):
                if (img[self.configs.SRC_ATTR].endswith(endswith_condition)
                        and img[self.configs.SRC_ATTR].startswith(startswith_condition)
                        and any(substring in img[self.configs.SRC_ATTR] for substring in substring_condition)
                ):
    ...
    def fetch_all_and_save(self,
        ...
        :param image_url_col_name: The name of the image url column in the output csv file.
        :param endswith_condition: The condition that the image url should end with.
        :param startswith_condition: The condition that the image url should start with.
        :param substring_condition: The condition that the image url should contain.
        :param driver_wait_time: The time to wait for the driver to load the page.
        :param sleep_time: The time to sleep between fetching image urls from each url.
        :return: None
        """
        ...
        for url in url_list:
            image_urls = self.fetch_one(
                driver=driver,
                url=url,
                endswith_condition=endswith_condition,
                startswith_condition=startswith_condition,
